# CONJUNTOS O SETS: colección (array) que permite almacenar múltiples elementos en una sola variable.
# Características: coleccion desordenada de elementos únicos y mutables* (NO ORDENADOS).
...
telefonos = {'Xiaomi', 'Samsung', 'Motorola', 'Honor'}
telefonos2 = ['Huawei', 'LG', 'Nokia', 'Xiaomi' ] #puede ser cualquier colección

telefonos.add("Iphone") # Agregar un elemento (si se agrega un elemento que ya existe, este no aparece).
...
autos = { 'Ford', 'Peugeot', 'Fiat', 'Renault', 'Ferrari', 'Audi', 'Suzuki' }
...
# Join de conjuntos (Teoría de conjuntos).
...
u2 = a | b # es exactamente lo mismo que usar union (a o b = todos).
print(u)
print(u2)

# a.update(b) # update modifica el conjunto original.

# Intersección: Devuelve los elementos que tengan en común ambos conjuntos sin modificar el conjunto original.
i = a.intersection(b)
...
# Diferencia simétrica: develover los elementos que no estén presentes en ambos conjuntos.
