# LISTAS O ARRAYS: colección que permite almacenar múltiples elementos en una sola variable.
...
acceder = frutas[2] # acceder por el indice.
longitud = len(frutas)  # saber la cantidad de elementos que tiene.

listaStrings = ["alfa", "beta", "gamma"]
listaNumbers = [0, 1, 2, 3, 4]
listaBooleans = [True, True, False]
# admite distintos tipos de datos en un solo array.
...
naranja = frutas[0]
parteLista = frutas[1:3] # Desde el 1(manzana) hasta el 3 (no incluido).
desdeComienzo = frutas[:2] # Desde el comienzo hasta el dos no inclúido.
desdeComienzo = frutas[:-2]# Desde el comienzo hasta dos indices antes del final.
...
tecnologias.extend(frontend) # Agregar cualquier tipo de colección a un lista (fusión) al final de ella.
# En el caso de un diccionario solo agrega la key.
...
# Ordenar una lista:
tecnologias.sort() # En caso de no especificar ordenara alfabéticamente o de forma ASCENDENTE.
numeros = [7,9,5,3,8,6,4,2]
numeros.sort(reverse=True) # En este caso ordenara de forma DESCENDENTE.
